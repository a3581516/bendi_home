...
class TestRegister(unittest.TestCase):
    ...
    def test_register(self,case):
        ...
        #
        # # 1）data的一个参数进行判断
        # if json.loads(case.datas)['mobilephone'] == 'register_mobile':
        #     ##如果找到 就换，先取数据库中最大的手机号码+1  作为 新的注册手机号
        #     new_num = int((MyDB().get_one('select max(mobilephone) from future.member'))[0])+1
        #     ##replace(字符串替换函数实现)   换后是新的字符串   需要重新 赋值给 case.datas =
        #     case.datas = case.datas.replace('register_mobile',str(new_num))
