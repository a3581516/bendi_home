...
from WS_API.common.contants import *
...
def create_ip():
    ...
    # ip = '.'.join('%s'%random.randint(1, 254) for i in range(4))
    ip = faker.ipv4()
    return ip
...
def random_card():
    # 随机生成银行卡号
    # card = '6214' + "".join(random.choice("0123456789") for i in range(12))
...
def create_card(start_with = '622609', total_num = 16):
    card = start_with
    # 随机生成前N-1位
    while len(card) < total_num - 1:
        card += str(random.randint(0, 9))
    # 计算前N-1位的校验和
    s = 0
    card_num_length = len(card)
    for _ in range(2, card_num_length + 2):
        t = int(card[card_num_length - _ + 1])
        if _ % 2 == 0:
            t *= 2
