...
from test_interface.common import http_request
...
class DoExcel():
    ...
    def get_dict_cases(self):
        ...
        for row in range(2, max_row + 1):
            ...
            dict['datas'] = self.sheet.cell(row=row,column=4).value
...
if __name__ == '__main__':
    from test_interface.common import contants
    do_excel=DoExcel(filename=contants.wscase_file,sheet_name='sendMCode')
    ...
    #     resp=http2.request(method=case.method,url=case.url,datas=eval(case.datas))
    ...
    # resp2 = http2.request('POST','http://test.lemonban.com/futureloan/mvc/api/member/recharge', datas=params)
    # http2.close()
