...
from test_UI.common.dir_config import log_dir
...
class MyLog:
    ...
    def my_log(self, msg, level):
        ...
        # 输出到 日志文件  "%Y-%m-%d %H:%M:%S"
        curTime = time.strftime("%Y-%m-%d", time.localtime())
        fh = logging.FileHandler(filename=log_dir+'/test_UI{0}.log'.format(curTime), encoding='utf-8')  # 默认 mode='a'g
        fh.setLevel('DEBUG')
